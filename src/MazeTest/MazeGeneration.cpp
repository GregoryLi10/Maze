	...
	for (int i=0; i<height; i++) {
		for (int j=0; j<width; j++) {
			if (i==0&&!(maze[i][j].find(UP)!=maze[i][j].end())){
				h.push_back(j*depth*100);
				h.push_back(i*depth*100);
			}
			if (!(maze[i][j].find(DOWN)!=maze[i][j].end())){
				h.push_back(j*depth*100+depth/10);
				h.push_back((i+1)*depth*100);
			}
			if (j==0&&!(maze[i][j].find(LEFT)!=maze[i][j].end())){
				v.push_back(j*depth*100); 
				v.push_back(i*depth*100);
			}
			if (!(maze[i][j].find(RIGHT)!=maze[i][j].end())){
				v.push_back((j+1)*depth*100);
				v.push_back(i*depth*100+depth/10);
			}
		}
	}
